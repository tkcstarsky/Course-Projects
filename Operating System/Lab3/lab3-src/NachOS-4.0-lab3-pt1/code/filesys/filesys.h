...
class FileSystem {
  public:
    FileSystem(bool format);		// Initialize the file system.初始化
    ...
    bool Create(char *name, int initialSize);  	
					// Create a file (UNIX creat)新建文件，带入文件名及大小

    OpenFile* Open(char *name); 	// Open a file (UNIX open) 用文件名打开文件

    bool Remove(char *name);  		// Delete a file (UNIX unlink)用文件名删除文件
	...
	/* ++++ LAB3 ++++ */
	int Close(int fileId);//由文件id关闭文件

	int Read(char* buffer, int size, int fid);//从fid文件读入size长度的字符串到buffer
  ...
  private:
    OpenFile* freeMapFile;		// Bit map of free disk blocks,
					// represented as a file打开存储位图的文件
    OpenFile* directoryFile;		// "Root" directory -- list of 
					// file names, represented as a file打开目录文件
    //LAB3
    map<int, OpenFile*> openedFile;    //opened file table已经打开的文件键值对
};
