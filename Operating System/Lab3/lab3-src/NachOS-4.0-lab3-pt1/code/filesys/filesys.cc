...
FileSystem::FileSystem(bool format)
{
    ...
    if (format)//需要初始化时进入分支
    {
        PersistentBitmap *freeMap = new PersistentBitmap(NumSectors);//新建位图
        Directory *directory = new Directory(NumDirEntries);//新建目录
        FileHeader *mapHdr = new FileHeader;//新建位图指针
        FileHeader *dirHdr = new FileHeader;//新建目录指针

        DEBUG(dbgFile, "Formatting the file system.");

        // First, allocate space for FileHeaders for the directory and bitmap
        // (make sure no one else grabs these!)
        freeMap->Mark(FreeMapSector);//将位图中0设置为位图指针
        freeMap->Mark(DirectorySector);//将位图中1设置为目录指针

        // Second, allocate space for the data blocks containing the contents
        // of the directory and bitmap files.  There better be enough space!

        ASSERT(mapHdr->Allocate(freeMap, FreeMapFileSize));//为两个基本文件分配文件空间
