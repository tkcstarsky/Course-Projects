...
int
Disk::TimeToSeek(int newSector, int *rotation) //计算寻道时间
{
    ...
    *rotation = 0;
    if (over > 0)	 	// if so, need to round up to next full sector
   	*rotation = RotationTime - over;//保存一下结束时所需回到最初磁道的时间
    return seek;
}
...
Disk::ComputeLatency(int newSector, bool writing)
{
    ...
    rotation += ModuloDiff(newSector, timeAfter / RotationTime) * RotationTime;//同磁盘上不同跨越磁道的时间

    DEBUG(dbgDisk, "Request latency = " << (seek + rotation + RotationTime));
    return(seek + rotation + RotationTime);//返回本次访问磁盘所需的总时间
}
