...
void ExceptionHandler(ExceptionType which)
{
    ...
    case SyscallException:
        ...
        case SC_Exec:
        {
            ...
            { //begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
			int namebase = (int)kernel->machine->ReadRegister(4);
            int value = 1;
            char ExecName[128];

            for(int i=0;i<128,value != 0;i++){
                kernel->machine->ReadMem(namebase+i,1,&value);
                ExecName[i]=(char)value;
            }//从寄存器中读入文件名
            SysExec(ExecName);//将文件名带入SysExec中做下一步处理
        ...
        {
            { //begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
				int child_id = (int)kernel->machine->ReadRegister(4); //取子进程id
                // 通过调用当前进程(父进程)的join函数，来等待指定的子进程
                // kernel->currentThread->join(id);//join()等待你实现
				kernel->currentThread->join(child_id);
