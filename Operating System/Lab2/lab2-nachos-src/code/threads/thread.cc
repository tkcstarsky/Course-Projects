...
// TODO:通过实现下列函数，来完成对join操作的支持（提示:利用信号量以及joinSemMap成员变量）
//begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int Thread::getTid()
{
    return tid;//添加函数返回进程tid
}
...
Thread::Thread(char *threadName)//进程的初始化函数
{
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    ...
    priority = CREATE_PRIORITY + random() % 50;//线程优先级=200+随机数<50

    { // begin:如果需要改它，请把请写在这里,如果不需要，请忽视++++++++++++++++
    //添加本段用以给生成的进程一个随机的tid
...
Thread::~Thread()//线程的析构函数
{
    { //begin: 如果需要改它，请把代码请写在这里,如果不需要，请忽视+++++++++++++++++++++++++
       kernel->removeThread(tid);
       parent = NULL;
    } //end: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
void Thread::Print()//用以打印进程的tid，名称，状态，优先级
{
    fprintf(kernel->thread_sch_f, "%12s=%d\t name=%-15s status=%-7s\t pri=%d\n", "tid", tid, name, threadStatusStr[status], priority);
}
...
void Thread::Fork(VoidFunctionPtr func, void *arg)//用以给子进程分配栈空间
{
    Interrupt *interrupt = kernel->interrupt;//中断
    Scheduler *scheduler = kernel->scheduler;//调度
    IntStatus oldLevel;
...
void Thread::Finish()
{
    ...
    { //begin: 如果需要，请把实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
       if(parent != NULL){
        Semaphore * semaphore = parent->joinSemMap_getSemByID(tid);
...
{
    stack = (int *)AllocBoundedArray(StackSize * sizeof(int));//StackSize = (8 * 1024)，生成对应大小的堆栈数组？
...
void Thread::SaveUserState()//保存寄存器状态函数
{
...
void Thread::RestoreUserState()//恢复寄存器状态
{
    for (int i = 0; i < NumTotalRegs; i++)
...
int Thread::joinSemMap_insert(int tid, Semaphore *sem)
{
    pair<map<int, Semaphore *>::iterator, bool> ret = joinSemMap.insert(pair<int, Semaphore *>(tid, sem));
    if (!ret.second)
        return -1;
    else
        return 1;
...
int Thread::joinSemMap_remove(int tid)
{
    if (joinSemMap.find(tid) == joinSemMap.end())
        return -1;
    else
    {
        joinSemMap.erase(tid);
        return 1;
    }
}

Semaphore *Thread::joinSemMap_getSemByID(int tid)
{
    map<int, Semaphore *>::iterator iter = joinSemMap.find(tid);

    if (iter == joinSemMap.end())
    {
        return NULL;
    }
    else
