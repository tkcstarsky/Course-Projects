...
{
    ListIterator<Thread *> ite(readyList);
    int pri;
    while (!ite.IsDone())
    {
      pri=ite.Item()->getPriority() + AdaptPace;
      ite.Item()->setPriority(pri);
        ite.Next();
    }
}
//end: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
{
    ASSERT(kernel->interrupt->getLevel() == IntOff);
    int currentpri,nowSwitchTick,Tick,Maxpri;
    //ListIterator<Thread *> ite(readyList);
    ...
    else
    {
        ...
        // TODO：
        //更新当前线程的优先级
        nowSwitchTick=kernel->stats->totalTicks;
	Tick=nowSwitchTick-lastSwitchTick;
        currentpri=kernel->currentThread->getPriority()-Tick/100;
	kernel->currentThread->setPriority(currentpri);
